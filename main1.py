...
# Falta arreglar espacio de mas en tk cadenas
# Falta agregar numeros
...
def lexical_analysis(input_file):
    ...
    str_re = re.compile('"[A-Za-z\s]*[0-9]*"')
    ...
    for idx, line in enumerate(input_string.splitlines()):
        ...
        for jdx, char in enumerate(line):
            if (char not in list(x for x in dict_conversor2.keys()) and char != "#"):

                if char == "\"":
                    if len(possible_substr) > 0:
                        if possible_substr[-1] != "\\":
                ...
                if possible_substr in list(x for x in dict_conversor2.keys()) and len(
                        possible_substr) == 2:
                    global_keywords_appear.append(
                        (dict_conversor2[possible_substr], idx + 1, jdx))
                    possible_substr = ""
                elif possible_substr + char in list(x for x in dict_conversor2.keys()) and len(
